...
class BotWrapper:
    ...
    def bootstrap_db(cur: sqlite3.Cursor) -> None:
        cur.execute(
            "CREATE TABLE IF NOT EXISTS messages(message_id VARCHAR(255) UNIQUE, channel_id, jump_id, user_id, reaction_count)"
        )
    ...
    def get_emoji_name(emoji) -> str:
        return (
            ...
            or isinstance(emoji, discord.PartialEmoji)
            else emoji
        )
    ...
    def calc_keks(reactions: List[discord.Reaction]) -> int:
        return sum(
            [
                r.count
                for r in reactions
                if re.match("^kek", BotWrapper.get_emoji_name(r.emoji).lower())
                is not None
            ]
        )

    async def update_tracked_messages_for_channel(
        ...
        if isinstance(channel, discord.TextChannel):
            ...
            messages = [
                ...
                async for message in channel.history(
                    after=three_days_ago, oldest_first=True
                )
                if (self.calc_keks(message.reactions) if len(message.reactions) else 0)
                > 0
            ]
            self.tracked_messages = {
                m.id: {"message": m, "reactions": m.reactions} for m in messages
            }
            for message in messages:
                print(
                    f"{message.author} said {message.content} in {channel.name} and got {len(message.reactions)} reactions"
                )
            ...
            message_tuples = [
                (
                    ...
                    message.jump_url,
                    message.author.id,
                    self.calc_keks(message.reactions),
                )
                for message in messages
            ]

            self.cursor.executemany(
                "INSERT OR REPLACE INTO messages VALUES(?,?,?,?,?)", message_tuples
            )

    async def update_counts_for_channels(self):
        channels = self.client.get_all_channels()
        await asyncio.gather(
            *[self.update_tracked_messages_for_channel(channel) for channel in channels]
        )
        ...
        async def on_message(msg: discord.Message):
            if msg.content.startswith("!getNums"):
                ...
                for (
                    ...
                    "SELECT * FROM messages ORDER BY reaction_count DESC LIMIT 5"
                ).fetchall():
                    ...
                    embedVar = discord.Embed(
                        title=f"This gem got by {author.display_name}", color=0x00FF00
                    )
